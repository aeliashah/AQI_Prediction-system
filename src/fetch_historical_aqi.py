...
# #     if response.status_code != 200:
# #         print("Error fetching AQI:", response.text)
# #         return
...
# #     file_path = os.path.join(DATA_DIR, "aqi_data.csv")
...
if not OW_API_KEY or not HOPSWORKS_API_KEY:
    raise Exception("❌ Missing API Keys — Check .env or GitHub Secrets")
...
# ========================================
# 2️⃣ Fetch last 1 year AQI data
# ========================================
end = datetime.utcnow()
start = end - timedelta(days=365)

print(f"🌍 Fetching Karachi AQI {start.date()} ➝ {end.date()}")
...
data = r.json()
if "list" not in data:
    print("❌ Invalid API structure:", data)
    exit()
...
# continuous features as float64 (double)
df["pm2_5"] = df["pm2_5"].astype(float)
df["pm10"] = df["pm10"].astype(float)
df["co"] = df["co"].astype(float)
df["no2"] = df["no2"].astype(float)
df["o3"] = df["o3"].astype(float)
df["aqi_diff"] = df["aqi_diff"].astype(float)
df["aqi_rolling_3h"] = df["aqi_rolling_3h"].astype(float)
df["aqi_roc_24h"] = df["aqi_roc_24h"].astype(float)
df["pm25_pm10_ratio"] = df["pm25_pm10_ratio"].astype(float)
...
# ========================================
os.makedirs("data", exist_ok=True)
df.to_csv("data/karachi_aqi_historical.csv", index=False)
print("💾 Saved data/karachi_aqi_historical.csv")

# ========================================
# 5️⃣ Upload to Hopsworks
# ========================================
print("🔗 Connecting to Hopsworks...")
...
fg = fs.get_or_create_feature_group(
    ...
    primary_key=["datetime"],
    event_time="datetime",
    description="1-year Karachi AQI history with features"
)
